...
def self.all_students_in_grade_X(grade)
  ...
  def self.first_student_in_grade_10
      ...
      SQL
      new_from_db(DB[:conn].execute(sql).first) #will return student instance
  end
  ...
  def self.find_by_name(name)
    ...
    SQL
    student = DB[:conn].execute(sql, name).first #pulls in student, assigns to variable, passes into above method
    new_from_db(student)
  end
